...
type DatanodeServer struct {
	...
	gossipAddrs   []string
	gossipClients map[string]dpb.DatanodeServiceClient
}
...
func NewDatanodeServer() *DatanodeServer {
	...
	s := &DatanodeServer{
		...
		gossipClients: make(map[string]dpb.DatanodeServiceClient),
...
func (s *DatanodeServer) dialPeer(addr string) dpb.DatanodeServiceClient {
	...
	if err != nil {
		log.Printf("[Datanode %s] No se pudo conectar a peer %s: %v", s.id, addr, err)
		return nil
	}
